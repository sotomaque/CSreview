...
function handleDurationConstraint(results, duration) {
  ...
  results.forEach((tuple) => {
    let availablityInMinutes = getTimeDifferenceOfStringInMinutes(
      tuple[0],
      tuple[1]
    );
...
function reduceMergedArrays(mergedArrays) {
  ...
  for (let i = 0; i < mergedArrays.length - 1; i++) {
    ...
    if (mergedArrays[i][1] > mergedArrays[i + 1][0]) {
      let endOfSlot1 = Math.max(
        processTimeFromStringToNumOfMinutes(mergedArrays[i][1]),
        processTimeFromStringToNumOfMinutes(mergedArrays[i + 1][1])
      );
...
let possibleMeetingTimes = suggestMeetingTime(
  cal1,
  cal2,
  bound1,
  bound2,
